...
public class LoginActivity extends BaseActivity implements View.OnClickListener {
   ...
   /*     gps = new GPSTracker(LoginActivity.this,LoginActivity.this);
        ...
        id = prefManager.getString("transport_id");
        prefManager.closeDB();
        ...
        callLogin(mobile,password).enqueue(new Callback<ResponseResult>() {
            ...
            public void onResponse(Call<ResponseResult> call, Response<ResponseResult> response) {
                ...
                if (Integer.parseInt(response.body().getResponse()) == 101) {
                    try {
                        Toasty.success(getApplicationContext(), "Welcome " + response.body().getLogin().get(0).getTransport_name(), Toast.LENGTH_SHORT).show();
                    ...
                    /* mTTS.speak( "Welcome to swack transport", TextToSpeech.QUEUE_ADD, null, null);
                    mTTS.setPitch((float)0.8);
                    mTTS.setSpeechRate((float)0.2);*/
                    prefManager.connectDB();
                    prefManager.setBoolean("isLogin", true);
                    prefManager.setString("transport_id", response.body().getLogin().get(0).getTransport_id());
                    prefManager.setString("transport_name", response.body().getLogin().get(0).getTransport_name());
                    prefManager.setString("transport_address", response.body().getLogin().get(0).getTransport_address());
                    prefManager.setString("transport_email", response.body().getLogin().get(0).getTransport_email());
                    prefManager.setString("transport_mob", response.body().getLogin().get(0).getTransport_mob());
                    prefManager.setString("transport_pic", response.body().getLogin().get(0).getTransport_profile());
                    prefManager.setString("transport_aadhar", response.body().getLogin().get(0).getTransport_aadhar());
                    prefManager.setString("transport_pan", response.body().getLogin().get(0).getTransport_pan());
                    prefManager.setString("transport_shopact", response.body().getLogin().get(0).getTransport_shopact());
                    prefManager.closeDB();

                    System.out.println("Login ID : "+response.body().getLogin().get(0).getTransport_id());
        ...
        apiService.forgotPassword(APIUrl.KEY,
            ...
            public void onResponse(Call<ResponseResult> call, Response<ResponseResult> response) {
                ...
                try {
                    if (Integer.parseInt(response.body().getResponse()) == 101) {
                        dialog.dismiss();
                        id = response.body().getLogin().get(0).getTransport_id();
                        otpDialog();

...
public class CompleteTransportAdapter extends RecyclerView.Adapter<CompleteTransportAdapter.MyViewHolder> {
    ...
    public CompleteTransportAdapter(HistoryActivity context,String transid) {
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, final int position) {
        ...
        holder.txtConfirmcom.setText(mOrder.getStatus_name());
       ...
       holder.txtConfirmcom.setOnClickListener(new View.OnClickListener() {
           @Override
           public void onClick(View v) {

               if(mOrder.getStatus_name().equalsIgnoreCase("New")){
                   holder.txtConfirmcom.setText("Confirm");
                   confirmOrder(mOrder);
               }else if(mOrder.getStatus_name().equalsIgnoreCase("On Process")){
                   holder.txtConfirmcom.setText("Submit Bill");
                   mCallback.onHandleSelection(mOrder);
               }else if(mOrder.getStatus_name().equalsIgnoreCase("Complete")){
                   holder.txtConfirmcom.setText("Complete");
                   // mCallback.onHandleSelection(mOrder);
               }
               else{
                   holder.txtConfirmcom.setVisibility(View.GONE);
               }
           }
       });
    }
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public TextView txt_joborderid,txt_jobid,txtJobDate,txtJobUnique,txtJobCustomer,
                txtJobDistance,txtJobLocation,txtConfirmcom,txtJobMobile;
        ...
        public MyViewHolder(View view) {
            ...
            txtConfirmcom = view.findViewById(R.id.txtConfirmcom);
    ...
    private void confirmOrder(final TransportList mOrder) {
        ...
        context.apiService.transportConfirm(APIUrl.KEY, transid, mOrder.getTransport_id(),mOrder.getTrnreqcusid()).enqueue(new Callback<ResponseResult>() {
            @Override
            public void onResponse(Call<ResponseResult> call, Response<ResponseResult> response) {
                System.out.println("Complete Transport Confirm response " + response.body().getResponse());
                progressDialog.dismiss();
                if (Integer.parseInt(response.body().getResponse()) == 101) {
                    Toasty.success(context, "Complete Transport Order Confirm", Toast.LENGTH_LONG).show();
